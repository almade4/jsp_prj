...
<body>
  ...
  ================================================== -->
		...
		<div class="container marketing">
			<!-- <form태그에 action속성을 정의하지 않으면 submit이 되었을 때
			자신 페이지를 재 요청하게된다 -->
			<form method="post">
			<label>이름</label><input type="name" name="name" value="테스트"/>
			<br>
			<label>관심언어</label>
			<%
			...
			pageContext.setAttribute("nowYear", ld.getYear());
			%>
			...
			<c:out value="${subject }"/>
			<c:if test="${i.last or i.first }"></span></c:if>
			</c:forEach>
			<br>
			...
			<tr>
				...
				<td><c:out value="${uDTO2.age } / ${nowYear-uDTO2.age} "/></td>
			</tr>
			<c:if test="${i.last }">
			<tr>
