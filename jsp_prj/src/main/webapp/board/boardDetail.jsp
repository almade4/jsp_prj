...
<body>
	...
	<main>
		<div id="divWriteForm" style="margin-top: 60px;">
			<%
			...
			if (cntVal.intValue() != num) {
				bs.modifyCount(num);
			}

			session.setAttribute("cntNum", num);
			%>
			<form method="post" name="writeForm" id="readForm">
				<input type="hidden" name="num" value="${bDTO.num }" /> <input
					type="hidden" name="currentPage" value="${param.currentPage }" />
				<table>
					<tr>
						<th colspan="2" style="text-align: center"><h3>아무말 대잔치
								글읽기</h3></th>
					</tr>
					<tr>
						<td width="120px">제목</td>
						<td><input type="text" name="title" id="title"
							style="width: 600px" value="${bDTO.title }"></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea name="content" id="content"><c:out
									value="${bDTO.content }" escapeXml="true" /></textarea></td>
					</tr>
					<tr>
						<td>작성자</td>
					...
					<tr>
						...
						<td><fmt:formatDate value="${bDTO.inputDate }"
								pattern="yyyy-MM-dd EEEE kk:mm:ss" /></td>
					</tr>
					<tr>
						<td colspan="2" align="center"><c:if
								test="${userInfo.id == bDTO.id }">
							...
							<a href="javascript:location.href='boardList.jsp?${queryString }'" class="btn btn-info btn-sm">리스트</a></td>
					</tr>
				</table>
			</form>
